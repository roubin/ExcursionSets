...
p.geometrical.size=1e15;
p.hitting_set.thresholds=f_set_thresholds(-10,10,0.1);


%% percolation
dimensions=[2:20]';
list_to_save=zeros(size(dimensions,1),2);
for d=1:size(dimensions,1)
    p.geometrical.spatial_dimension=dimensions(d,1);
    ...
    display(['d=' num2str(p.geometrical.spatial_dimension)])
    ...
    if(size(min_ec)>0)
        if(max_ec(end,2)*min_ec(end,2)<0)
            ...
            list_to_save(d,1)=x01;
            list_to_save(d,2)=th_vo(x01);
        end
    end
    
    figure(3); semilogx(dimensions,list_to_save(:,2));
end

%% Gnuplot Output
TAB=[dimensions list_to_save];
figure(3); semilogx(TAB(:,1), TAB(:,3), 'b-*')
output_file_folder='.'; output_file_name=['output/percolation_ND_infinite_lastRoot.dat'];
txt1='Spatial dimention N; threshold+; fraction volume+';
