...
%% th ELKC
input_file_folder='input'; input_file_name='gaussian_2D.es';
...
for l=1:size(lengths,1)
    ...
    th_vo=@(t) f_elkc(2, p.geometrical, p.rf_distribution, p.rf_correlation, struct('type',p.hitting_set.type,'thresholds',t))./p.geometrical.size^2;
    ...
    if(nmin==1 && nmax==2)
        if(min_ec(2)<0)
            ...
            [x01, y01]=fzero(th_ec,[-10000, min_ec(1)]);
            [x02, y02]=fzero(th_ec,[max_ec(2,1), min_ec(1)]);
            %figure(1); plot(tr,ec,[max_ec(:,1); min_ec(1)],[max_ec(:,2); min_ec(2)], '+r', [x01 x02],  [y01 y02], '*r', [min(tr) max(tr)], [0 0], '--k');
...
output_file_folder='.'; output_file_name='output/percolation_2D.dat';
