...
%% read ref
rf_file_name='gaussian_random_field_2D.txt';
random_fields=f_read_random_field(input_file_folder, rf_file_name);
random_fields.Values=exp(random_fields.Values);

%% Experiment LKC
input_file_name='experimental_cumulative_2D.es';
p_ex=f_read_input(input_file_folder, input_file_name);
p_ex.hitting_set.thresholds=f_set_thresholds(-2,3,0.1,'log10');
ex_lkc=f_exp_lkc(random_fields, p_ex.hitting_set);

%% Plot
figure(1); semilogx(p.hitting_set.thresholds, th_elkc(2,p.hitting_set), p_ex.hitting_set.thresholds, ex_lkc.ELKC2, '-+');
