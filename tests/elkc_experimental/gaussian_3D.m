...
%% Experiment LKC
input_file_name='experimental_tail_3D.es';
p_ex=f_read_input(input_file_folder, input_file_name);
p_ex.hitting_set.thresholds=f_set_thresholds(-5,5,0.1);
ex_lkc=f_exp_lkc(random_fields, p_ex.hitting_set);

%% Plot
%figure(1); plot(p.hitting_set.thresholds, th_elkc(3,p.hitting_set), p_ex.hitting_set.thresholds, ex_lkc.ELKC3, '-+');
%figure(2); plot(p.hitting_set.thresholds, th_elkc(2,p.hitting_set), p_ex.hitting_set.thresholds, ex_lkc.ELKC2, '-+');
%figure(3); plot(p.hitting_set.thresholds, th_elkc(1,p.hitting_set), p_ex.hitting_set.thresholds, ex_lkc.ELKC1, '-+');
%figure(4); plot(p.hitting_set.thresholds, th_elkc(0,p.hitting_set), p_ex.hitting_set.thresholds, ex_lkc.ELKC0, '-+');
...
txt2='threshold min(LKC0) mean(LKC0) max(LKC1) min(LKC1) mean(LKC1) max(LKC1) min(LKC2) mean(LKC2) max(LKC2) min(LKC3) mean(LKC3) max(LKC3)';
f_write_gnuplot_output(output_file_folder, output_file_name, p_ex.hitting_set.thresholds, [ex_lkc.ELKC0, ex_lkc.ELKC1, ex_lkc.ELKC2, ex_lkc.ELKC3], txt1, txt2);
