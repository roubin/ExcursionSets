function elkc=f_elkc_derivatives(j_lkc, geometrical, rf_distribution, rf_correlation, hitting_set)
...
for i_gmf=0:(geometrical.spatial_dimension-j_lkc)
    ...
    elkc=elkc+f_flag_coef(i_gmf+j_lkc,i_gmf)*(lam2/(2.*pi))^(double(i_gmf)/2.)*lkc(i_gmf+j_lkc)*gmf(i_gmf);
end
