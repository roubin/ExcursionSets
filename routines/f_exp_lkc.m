...
switch rf_dimension
    ...
    case 3 % WARNING in 3D points_in is defined by and array of 1 and 0
        lkc=struct('ELKC0',zeros(n_thresholds,3),'ELKC1',zeros(n_thresholds,3), 'ELKC2',zeros(n_thresholds,3), 'ELKC3',zeros(n_thresholds,3), 'LKC0',zeros(n_thresholds,n_rea), 'LKC1',zeros(n_thresholds,n_rea), 'LKC2',zeros(n_thresholds,n_rea), 'LKC3',zeros(n_thresholds,n_rea));
        ...
        for i_threshold=1:n_thresholds
            ...
            for i_rea=1:n_rea
                ...
                sX=random_fields.SizeX;
                sY=random_fields.SizeY;
                sZ=random_fields.SizeZ;
                n_points_in=sum(points_in,1);
                ...
                if(~skipl0)
                    ...
                    F=0; % number of faces
                    V=0; % number of faces
                    nX=random_fields.NpointsX;
                    nY=random_fields.NpointsY;
                    nZ=random_fields.NpointsZ;
                    %N=round(n_points^(1/2));
                    for a3=1:nZ
                        for a2=1:nY
                            for a1=1:nX
                                S = S + points_in(a1+nX*(a2-1)+nX*nY*(a3-1));
                                if( points_in(a1+nX*(a2-1)+nX*nY*(a3-1)) )
                                    if( a1 < nX )
                                        A = A + points_in(a1+1+nX*(a2-1)+nX*nY*(a3-1));
                                    end
                                    if( a2 < nY )
                                        A = A + points_in(a1+nX*a2+nX*nY*(a3-1));
                                    end
                                    if( a3 < nZ )
                                        A = A + points_in(a1+nX*(a2-1)+nX*nY*a3);
                                    end
                                end
                                if( (a1<nX) && (a2<nY) )
                                    %Face du plan XY
                                    F = F+(points_in(a1+nX*(a2-1)+nX*nY*(a3-1))*points_in(a1+1+nX*(a2-1)+nX*nY*(a3-1))*points_in(a1+nX*a2+nX*nY*(a3-1))*points_in(a1+1+nX*a2+nX*nY*(a3-1)));
                                end
                                if( (a3<nZ) && (a2<nY) )
                                    %Face du plan YZ
                                    F = F+(points_in(a1+nX*(a2-1)+nX*nY*(a3-1))*points_in(a1+nX*(a2-1)+nX*nY*a3)*points_in(a1+nX*a2+nX*nY*(a3-1))*points_in(a1+nX*a2+nX*nY*a3));
                                end
                                if( (a1<nX) && (a3<nZ) )
                                    %Face du plan XZ
                                    F = F+(points_in(a1+nX*(a2-1)+nX*nY*(a3-1))*points_in(a1+1+nX*(a2-1)+nX*nY*(a3-1))*points_in(a1+nX*(a2-1)+nX*nY*a3)*points_in(a1+1+nX*(a2-1)+nX*nY*a3));
                                    if ( a2<nY )
                                        V = V+points_in(a1+nX*(a2-1)+nX*nY*(a3-1))*points_in(a1+nX*(a2-1)+nX*nY*a3)*points_in(a1+nX*a2+nX*nY*(a3-1))*points_in(a1+nX*a2+nX*nY*a3)*points_in(a1+1+nX*(a2-1)+nX*nY*(a3-1))*points_in(a1+1+nX*(a2-1)+nX*nY*a3)*points_in(a1+1+nX*a2+nX*nY*(a3-1))*points_in(a1+1+nX*a2+nX*nY*a3);
                                    end
                                end
                            end
                        end
            ...
            lkc.ELKC3(i_threshold,1)=min(lkc.LKC3(i_threshold,:));
            lkc.ELKC3(i_threshold,2)=mean(lkc.LKC3(i_threshold,:));
            lkc.ELKC3(i_threshold,3)=max(lkc.LKC3(i_threshold,:));
            if(~skipl0)
                lkc.ELKC0(i_threshold,1)=min(lkc.LKC0(i_threshold,:));
                lkc.ELKC0(i_threshold,2)=mean(lkc.LKC0(i_threshold,:));
                lkc.ELKC0(i_threshold,3)=max(lkc.LKC0(i_threshold,:));
